...
public class ActiveQuestSQLiteRepository extends SQLiteGenericRepository<UUID, ActiveQuestDto> implements ActiveQuestRepository {

    public ActiveQuestSQLiteRepository(SqliteDatabaseManager manager) {
    ...
    public CompletableFuture<Void> remove(QuestGroup questGroup) {
        return CompletableFuture.runAsync(() -> {
            try (Connection connection = ds.getConnection();
                 PreparedStatement statement = connection.prepareStatement("DELETE FROM " + tableName + " WHERE json_extract(data, '$.groupId') = ?")) {
    ...
    public CompletableFuture<Collection<ActiveQuestDto>> fetchByPlayer(UUID uuid) {
        return CompletableFuture.supplyAsync(() -> {
            try (Connection connection = ds.getConnection();
                 PreparedStatement statement = connection.prepareStatement("SELECT * FROM " + tableName + " WHERE json_extract(data, '$.owner') = ?")) {
