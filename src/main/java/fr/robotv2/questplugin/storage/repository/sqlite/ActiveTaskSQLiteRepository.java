...
public class ActiveTaskSQLiteRepository extends SQLiteGenericRepository<UUID, ActiveTaskDto> implements ActiveTaskRepository {

    public ActiveTaskSQLiteRepository(SqliteDatabaseManager manager) {
    ...
    public CompletableFuture<Void> remove(QuestGroup group) {
        return CompletableFuture.runAsync(() -> {
            try (Connection connection = ds.getConnection();
                 PreparedStatement statement = connection.prepareStatement("DELETE FROM " + tableName + " WHERE json_extract(data, '$.parentQuestGroupId') = ?")) {
    ...
    public CompletableFuture<Collection<ActiveTaskDto>> fetchByActiveQuest(UUID activeQuestId) {
        return CompletableFuture.supplyAsync(() -> {
            try (Connection connection = ds.getConnection();
                 PreparedStatement statement = connection.prepareStatement("SELECT * FROM " + tableName + " WHERE json_extract(data, '$.parentActiveQuestId') = ?")) {
    ...
    public CompletableFuture<Multimap<UUID, ActiveTaskDto>> bulkFetchByActiveQuest(Collection<UUID> activeQuestIds) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            String query = "SELECT * FROM " + tableName + " WHERE json_extract(data, '$.parentActiveQuestId') IN (" + placeholders + ")";
