...
public class ActiveTaskMariaDBRepository extends MariaDBGenericRepository<UUID, ActiveTaskDto> implements ActiveTaskRepository {

    public ActiveTaskMariaDBRepository(MariaDbDatabaseManager manager) {
    ...
    public CompletableFuture<Void> remove(QuestGroup group) {
        return CompletableFuture.runAsync(() -> {
            try (Connection connection = ds.getConnection();
                 PreparedStatement statement = connection.prepareStatement("DELETE FROM " + tableName + " WHERE JSON_UNQUOTE(JSON_EXTRACT(data, '$.parentQuestGroupId')) = ?")) {
    ...
    public CompletableFuture<Collection<ActiveTaskDto>> fetchByActiveQuest(UUID activeQuestId) {
        return CompletableFuture.supplyAsync(() -> {
            try (Connection connection = ds.getConnection();
                 PreparedStatement statement = connection.prepareStatement("SELECT * FROM " + tableName + " WHERE JSON_UNQUOTE(JSON_EXTRACT(data, '$.parentActiveQuestId')) = ?")) {
                statement.setString(1, activeQuestId.toString());
    ...
    public CompletableFuture<Multimap<UUID, ActiveTaskDto>> bulkFetchByActiveQuest(Collection<UUID> activeQuestIds) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            String placeholders = String.join(",", Collections.nCopies(activeQuestIds.size(), "?"));
            String query = "SELECT * FROM " + tableName + " WHERE JSON_UNQUOTE(JSON_EXTRACT(data, '$.parentActiveQuestId')) IN (" + placeholders + ")";

            try (Connection connection = ds.getConnection();
                 PreparedStatement statement = connection.prepareStatement(query)) {
                int index = 1;
                for (UUID activeQuestId : activeQuestIds) {
                    statement.setString(index++, activeQuestId.toString());
                }
